...
Surprisingly, even female appears to have more depression, their suicide rate is much lower than male. Could this give a hint that women are more endurable to depression? We then continue to conduct formal statitical anaylsis hoping to gain evidence as justification to our findings based on the summary plots.
...
Next, we construct regression for suicide death rate as response, sex, race, age(in years), year as predictors. By observing the coefficients estimates and p-values, there is several interesting findings for suicide death rates:
...
5.  the model produced an adjusted *R*<sup>2</sup> = 0.6405, which represents there are 64% of the variability of the suicide rate are explained by the fitted model explanatory variables and it's a acceptable proportion to indicate a good fit for the data.
